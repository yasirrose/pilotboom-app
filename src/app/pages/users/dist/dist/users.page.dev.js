...
function () {
  ...
  UsersPage.prototype.loadUsers = function (event, refresh) {
    ...
    this.userApi.getUsersWithMeta(this.per_page, this.page).subscribe(function (res) {
      ...
      if (event && _this.search && _this.searchTerm) {
        //Pagination called and search exists already
  ...
  UsersPage.prototype.filterSearch = function (objArray, value) {
    ...
    return objArray.filter(function (obj) {
      return obj.data.display_name.toLowerCase().indexOf(value.toLowerCase()) > -1 || obj.data.user_login.toLowerCase().indexOf(value.toLowerCase()) > -1 || obj.data.user_nicename.toLowerCase().indexOf(value.toLowerCase()) > -1;
    });
  };

  __decorate([core_1.ViewChild(angular_1.IonContent)], UsersPage.prototype, "content");

  UsersPage = __decorate([core_1.Component({
    selector: 'app-users',
